...
| `Framing '<eigene SWA-URL>…' violates … frame-src` | **Nicht** die Anmelde-Domain fehlt, sondern die **eigene**: die stille Erneuerung endet mit einem Redirect auf die `redirectUri` — also zurück in die eigene Origin, im iframe | `'self'` in `frame-src`. Ohne das kann MSAL die Antwort im iframe nie lesen und der Fallback auf interaktives Anmelden greift nicht |
