...
> im Prototyp tut, was es am Fork (azure/powerapps) ersetzt, und ob der Austausch
...
| Mock-Adapter | `apps/web/src/data/adapters/mock/*` | In-Memory-Store implementiert die Ports | `AzureRepository` (eine Klasse für alle Entitäten, R4) bzw. `Dataverse*Repository` (`platform: powerapps`) am selben Port | `prototype` |
...
| Adapter-Swap-Punkt | `apps/web/src/data/index.ts` | Verzweigt auf `platform`; `mock` liefert die Mock-Repositories | `azure`-Zweig verdrahtet (`AzureRepository`), `powerapps` noch `unsupported()` | `forked` |
...
## Fork-Checkliste (beim Übergang mock → azure/powerapps)

1. `platform` in `.unitix/project.json` umstellen.
2. Vollstack-Adapter pro Entität am jeweiligen Port implementieren, `apps/web/src/data/index.ts` um den Zweig ergänzen.
